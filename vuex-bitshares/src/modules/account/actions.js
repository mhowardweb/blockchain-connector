...
const actions = {
  ...
  restoreBackup: async ({ commit }, { backup, password }) => {
   ...
   */
  signup: async (state, { name, password, dictionary, email }) => {
    const { commit } = state;
  ...
  storeBackupDate: (state, { date, userId }) => {
    const { commit } = state;
   ...
   */
  login: async (state, { password, brainkey }) => {
   ...
   */
  checkIfUsernameFree: async (state, { username }) => {
